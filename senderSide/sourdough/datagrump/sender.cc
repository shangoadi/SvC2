...
char buffaray_commline[PKT_SIZE];

int msqid_send,msqid_commline,msqid_commline_sent;
// key_t key;
struct mq_attr  obuf;
...
void* sender(void *parms)
{
  ...
  arg8 = arguments.front();
  arguments.pop_front();

    arg9 = arguments.front();
  arguments.pop_front();

  arg10 = arguments.front();
  arguments.pop_front();
  ...
  // int argc = arguments.size();
    ...
    attr2.mq_msgsize = sizeof(unsigned int)*10;
    attr2.mq_flags = 0;
    attr2.mq_maxmsg = 10;
             ...
             const ContestMessage & ack )
{
  ...
  unsigned int temp_window_size = controller_.window_size();
  std::string s = std::to_string(temp_window_size);
...
void DatagrumpSender::send_datagram( void )
{
  ...
  // if (mq_send(msqid_commline_sent, s.c_str(), sizeof(unsigned int), 0) == -1) /* +1 for '\0' */
  //    perror("error in msgsnd() commline");
